...
-- Creación de Single Index
CREATE INDEX salary_indx ON users (salary);
CREATE UNIQUE INDEX email_indx ON users (email);
-- DROP INDEX salary_indx

-- Creación de Multiple Index
CREATE INDEX multiaddr ON addresses(street, city);

-- Creación de Partial Indexes
CREATE INDEX salary_indx2 ON users (salary)
WHERE salary > 12000;
